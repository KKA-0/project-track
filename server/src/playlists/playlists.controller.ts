...
export class PlaylistsController {
  ...
  async getPlaylist(@Body() input: { playlistId: string }, @Headers('Authorization') token: string) {
    ...
    try {
      ...
      return { ...GenPlaylistData, isUser: (token !== undefined) || false };
    } catch (err) {
  ...
  @Post('video/status')
  async updateVideoStatus(@Body() body: { playlistId: string, section: string, videoTitle: string, checked: boolean, completedState: number }, @Headers('Authorization') token: string) {
    if (token !== undefined) {
      const TokenParts = token.split('Bearer ')
      token = TokenParts[1]
      const decodedToken = jwtDecode(token) as { id: number }
